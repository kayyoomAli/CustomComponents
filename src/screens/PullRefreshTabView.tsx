...
const App = () => {
   ...
   * stats
   */
  const [tabIndex, setIndex] = useState<any>(0);
  const [routes] = useState([
    {key: 'tab1', title: 'Tab1'},
    {key: 'tab2', title: 'Tab2'},
  ]);
  const [canScroll, setCanScroll] = useState<any>(true);
  const [tab1Data] = useState<any>(Array(40).fill(0));
  const [tab2Data] = useState<any>(Array(30).fill(0));

  /**
   * ref
   */
  const scrollY = useRef<any>(new Animated.Value(0)).current;
  const headerScrollY = useRef<any>(new Animated.Value(0)).current;
  ...
  const headerScrollStart = useRef<any>(0);
  const _tabIndex = useRef<any>(0);
  const refreshStatusRef = useRef<any>(false);

  /**
   * PanResponder for header
   */
  const headerPanResponder = useRef<any>(
    PanResponder.create({
